...
- Reaching maximum heat shall stall the engine for exactly 3.5 seconds (210 fixed steps) on the fixed simulation clock. Throughout the stall, Ride and Turbo shall not accelerate the bike, lane and pitch commands shall be ignored, and the bike shall coast down; heat shall keep cooling, and a cooling gate crossed during the stall shall still drop heat immediately but shall not shorten the stall. Control shall return on the first fixed step after it. Player and AI riders share this rule. Because every Solo/par/mastery calibration reference and every AI heat limit stays below overheat, the stall length shall not move those targets; a future change that lets a reference or AI rider overheat must recalibrate them.
- The stall shall be visible at the bike for its full length: a thick cloud of dark smoke and small fire/ember particles from the tailpipe, with the caption and hint naming the 3.5-second stall. These effects are presentation only, run on render time, hold still while paused, and never feed simulation. Reduced Motion keeps the smoke as the stall cue and suppresses the embers.
...
- A crash shall separate the rider from the bike: the rider is thrown clear and tumbles down the course while the bike falls onto its side and slides. The tumble is a deterministic, presentation-only effect seeded from the crash cause and the speed carried into the crash; it shall not use generic rigid-body physics or feed simulation, AI, timing, collision, or replay state. The authoritative bike still stops where the simulation crashed it and recovery respawns it there, so holding Recover shall hand the rider and bike back to the static crash-and-recover pose, reaching it exactly when recovery begins. Reduced Motion shall keep the static crash silhouette without the tumble.
...
AI riders shall also race the same course contract as the player: checkpoint count and lap count shall be derived once from the track and racing mode and given to every rider in the field, never written independently for the field. A field racing a different checkpoint or lap count is not racing the same course — it is being classified against a different one, and the disagreement is invisible in the result.
...
- Clock-raced modes (Solo, Practice, Custom) shall replay the player's fastest stored run for the exact course as a translucent personal ghost, and shall show a signed split delta against it measured at the player's current forward distance rather than at a shared clock. The ghost shall be presentation only: it shall hold no simulation, shall never collide, classify, or otherwise influence the live race, and shall never become gameplay authority. Where no stored run exists for that exact course, or the stored run cannot be decoded, the race shall start normally with no ghost and no delta. Where the player has ridden past every distance the ghost reached, the delta shall report that no comparison exists rather than extrapolating one. Rival and Mastery races shall field no ghost, and the delta's sign shall carry its meaning without relying on color.
...
- undo/redo preserving at least 50 actions, where a continuous gesture on the track name, lap count, or difficulty control costs one action rather than one per input event, and ends on blur so a later gesture on the same control is a separate action. The inspector's placement fields and stepper buttons are not yet coalesced and still cost one action per press;
...
- ghost retrieval shall select the fastest stored run bound to the exact course key, breaking ties on the earlier recording so a ghost stays stable across sessions. It shall skip records written before the course binding existed, and shall skip rather than surface a record that fails codec validation, falling through to the next-fastest usable run. A database-level failure shall be published through the standard persistence-failure channel and shall not prevent the race from starting;
...
- the application shall request the browser's persistent-storage bucket once, after the first confirmed profile write rather than during boot, so an engagement-based grant has evidence to judge and any prompting browser asks about data the player has actually created. The request shall be advisory only: no save, load, migration, recovery, or race path may depend on its result, a declined or unsupported request shall be treated as an expected outcome rather than a failure, and the negotiation shall never reject;
- the support screen shall disclose the resulting durability state truthfully. Without a grant it shall state that the browser may delete progress, tracks, and replays to reclaim space and that WebKit removes site data after seven days without a visit; with a grant it shall state that the browser will not clear those saves to reclaim space; where the browser does not report durability it shall advise treating local saves as temporary. On an iOS or iPadOS browser without a grant, and only when the game is not already running from the Home Screen, it shall additionally name Home Screen installation as the documented way to obtain a separate use counter. The disclosure shall expose its state as a data attribute for support and browser evidence;
...
At the shipped default UI scale and a 1280×800 viewport, no visible text shall render below a 12 CSS-pixel font size, and the personal-best and target strings the time-trial loop is read from shall render at no less than 13. The floor is asserted against shipped defaults rather than the 140% UI scale setting, because Steam Deck compatibility is judged on the configuration a player receives without opening settings; Valve's criterion is written about character height, and the condensed display face runs roughly 0.7 cap height, so 12 is the conservative reading of a 9-pixel character floor. Screen-reader-only copy is exempt because it is never rendered for sighted players.

One exemption is recorded rather than silently taken: below 780 CSS pixels the twelve tutorial progress markers keep a compact numeral. Twelve markers share a single row, so a 12-pixel numeral at 140% UI scale overflows its cell and breaks this section's existing requirement that all twelve stay readable without horizontal overflow on a 320×568 viewport. The marker's state is carried by its position and its border/fill treatment, and the list's accessible name is what announces progress; the numeral is redundant to both. The floor is unchanged at desktop widths, where the row has the room.
...
- The visual identity shall be bright, colorful, coherent, and original, with a polished 3D mascot-racer finish rather than Roblox-adjacent block assets.
- Forms shall keep strong gameplay readability while moving toward rounded silhouettes, richer authored materials, expressive surface detail, and lively trackside spectacle.
- The updated creative target may use family-friendly kart-racer polish as a quality reference, but the product shall not copy Mario Kart, Nintendo characters, items, UI, course layouts, vehicle silhouettes, music, sounds, iconography, branding, or trade dress.
...
- Race rendering shall establish its intended composition on the first frame, use per-track lighting, exposure, and fog profiles, and reinforce the premium arcade racing course with terraced edges, visible lane ridges, safety treatment appropriate to each venue, and deliberate scenery density.
...
- Player fallback and rival presentation riders shall use a bounded six-pivot presentation rig for torso, head, arms, and legs. Existing render snapshots may drive modest speed, airborne pitch/height, lane-lean, and static crash poses, plus the §5.6 crash tumble, but these transforms shall never feed simulation. Reduced Motion shall suppress the nonessential tuck, bob, cadence, jump, and lean articulation while retaining the essential crash silhouette.
