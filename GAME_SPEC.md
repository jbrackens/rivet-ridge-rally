...
**Latest-change validation:** `UNVERIFIED` while the automated and browser qualification run remains paused
...
For every non-tutorial race, the loading state shall hold the fixed-step simulation and race clock at zero until the essential rider asset either loads or selects its safe built-in fallback. Rider-asset readiness shall have a five-second deadline: a request that rejects or remains unsettled at that point shall be canceled through its isolated loader, shall select the already-present built-in bike, and shall terminate its texture-worker resources; any result that still settles late shall be discarded without changing the active race. Canyon Kickoff and Rider School shall also settle their original festival-canyon far-background load or its procedural-sky fallback within five seconds. A failed, unsupported, canceled, or late background request shall never block play, replace an active scene after readiness, or affect simulation. A visible `3 → 2 → 1 → GO` gate shall follow. The `GO` label shall be presented by the browser before the engine enters its racing phase; rider input and gameplay time shall begin on that immediate handoff, never before the player can see `GO` and never after an unexplained visual delay. Hiding the page during loading/countdown shall leave the race paused when the gate completes rather than starting unseen. Tutorial mode retains its explicit player-operated Lesson 1 start instead of stacking a second countdown over Rider School, and its engine shall remain paused until that action.
...
9. Read the named striped lane-choice barrier and move to an open lane before contact; avoiding an unrelated earlier hazard shall not satisfy this lesson.
...
- A simulation finish shall synchronously commit exactly one attempt-scoped result, progression update, and replay handoff before any cosmetic finish-presentation delay. Retrying or navigating away during that delay shall preserve the committed result without allowing a stale attempt to reopen results or overwrite a newer race.
...
- a failed background Test Ride save shall retain the exact attempted snapshot and disclose it until that snapshot is successfully retried or a same-track save/export explicitly supersedes it; a generic profile retry shall not hide that operation-specific failure;
...
- Every Rival and Mastery entrant shall use a coherent authored primary/accent palette, a distinct two-digit jersey and rear-plate number, and the same procedural knobby tread, brake, side-panel, and exhaust assemblies as the fallback player bike. Number rendering shall reuse one bounded runtime texture/material pair per rider, and the fixed five-rider field shall remain the allocation ceiling.
- Player and rival procedural riders shall use a bounded six-pivot presentation rig for torso, head, arms, and legs. Existing render snapshots may drive modest speed, airborne pitch/height, lane-lean, and static crash poses, but these transforms shall never feed simulation. Reduced Motion shall suppress the nonessential tuck, bob, cadence, jump, and lean articulation while retaining the essential crash silhouette.
...
- The application shall claim cached-race offline readiness only after the current page receives a successful cache acknowledgement from its exact current-generation service-worker controller. A later `controllerchange` shall immediately invalidate that page's readiness claim and restore conservative offline copy until the new current controller acknowledges the exact expected generation. Upgrade transitions may retain one immediately previous app-owned cache, but current-generation matches shall take priority and a failed or stale acknowledgement shall produce conservative offline copy rather than a false readiness promise.
- The current service-worker generation shall be `rivet-ridge-rally-shell-v30`. Installation shall fetch `index.html` once with reload semantics, cache clones of those exact bytes under both `/` and `/index.html`, and fetch core—including both bundled font subsets—plus HTML-discovered build assets before committing the current cache; any failed install shall delete the partial v30 cache. Runtime warming shall also request fresh same-origin bytes while retaining an already cached usable response if that refresh fails.
...
- Production smoke shall accept only a valid format-2 release manifest and a credential-free dedicated-origin root URL. It shall fetch every recorded file without cache reuse, compare each served byte count and SHA-256 plus the aggregate digest, then fetch `/` separately and require those bytes to equal the manifest's `index.html` without leaving the origin. Browser navigation shall remain on that origin, and the evidence shall bind the normalized root/entrypoint plus manifest source, toolchain, and build identity before exercising the release.
- Production smoke shall stage each run in a unique directory, emit schema-4 evidence that records each captured screenshot's safe relative path, byte count, and SHA-256 plus one path-sorted screenshot aggregate, catch setup/browser failures into an explicit failure record when the artifact root is writable, and atomically promote the completed bundle beneath the full manifest SHA-256 (or a separate unbound failure namespace). It shall never overwrite the retained schema-1 historical smoke files.
- Release-manifest generation shall prepend the active Node executable directory to the isolated child environment, verify the resolved child Node version, file identity, and SHA-256 before and after install/build, record the Node executable hash, and revalidate source, annotated tag, package/lock/Node inputs, npm CLI, and Node executable after sidecar creation and temporary-worktree cleanup before reporting success.
...
No requirement becomes `PASS` merely because code exists. It requires a named test or recorded manual procedure, environment, date, result, and evidence. The latest gameplay, responsive, accessibility, typography, shell-v30, and manifest-bound production-smoke changes remain `UNVERIFIED` while the requested test pause is active. A new release candidate is permitted only when mandatory requirements pass, except honestly documented hardware-specific `UNVERIFIED` cases and genuine external blockers.
