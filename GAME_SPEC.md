...
- A non-repeated lane-key press and release that both occur between fixed-step input samples shall still produce exactly one lane-change command on the next eligible sample; suspension, focus loss, disconnect, modifier cancellation, or a binding change shall discard any unsampled lane tap.
