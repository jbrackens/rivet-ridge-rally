...
That boundary is **enforced rather than described**. `eslint.config.js` applies `no-restricted-imports` and `no-restricted-globals` to `src/game/simulation/**`, `src/game/content/**`, `src/game/replay/**`, and the pure engine modules `aiRules.ts`, `crashRagdoll.ts`, `obstacleContacts.ts`, `racePresentation.ts` and `tutorialLessonGate.ts`, refusing `three`, `react`, `react-dom`, `zustand`, `dexie`, anything under `persistence/`, `ui/` or the app store, and the `window`, `document`, `navigator`, `localStorage`, `sessionStorage`, `indexedDB`, `requestAnimationFrame` and `performance` globals. Each refusal names the alternative. This is the property the rest of the architecture rests on: it is why a full race can be driven in a Node test with no canvas, why the replay codec and ghost fold are unit-tested at the fidelity the renderer sees, and why measured finish times are exact rather than estimated.

Facts that more than one module needs are declared once and imported. `LANE_POSITIONS` lives in `src/game/simulation/types.ts` and is imported by obstacle contacts, editor validation, authored-track conversion, and the editor scene — it previously existed as five independent copies, each deciding something load-bearing. The course contract a race runs under is derived once by `deriveRaceContract` in `aiRules.ts` and given to both the player simulation and every AI rider, so a field cannot be classified against a different checkpoint or lap count than the player races.
...
While a rider is crashed, the same nodes also carry a presentation-only crash tumble. `crashRagdoll.ts` is a pure, lint-bounded module that integrates two bodies — the rider's centre of mass and the bike — plus six flailing limb targets in fixed 1/120 s sub-steps of render time, seeded from the crash cause, the speed carried into the crash (which the simulation zeroes), and the crash position; equal render time therefore yields equal poses however frames are chunked. It is a hand-written tumble, not a rigid-body solver: Rapier is neither used nor a dependency, and nothing here decides vehicle feel. `GameEngine` places each body along `CoursePresentationRoute` from the rider group's unrolled course frame, converts the result into each node's parent space, and blends it over the static crash pose by a weight that is 1 while crashed and falls to 0 as Recover is held, so the static pose — and the unchanged simulation respawn point — is reached exactly when the simulation begins recovering. Reduced Motion forces the weight to 0. The bike node's rest transform is restored when the tumble ends; the authoritative bike never moves.

An overheat stall is likewise shown only at render time: pooled camera-facing smoke planes (normal alpha, per-particle opacity) and one additive instanced ember batch, emitted from the tailpipe opening found once per bike visual from its named exhaust geometry (bounds fallback for rivals). Emission follows `bike.overheated` from the simulation snapshot, uses render time held at zero while paused, carries part of the bike's speed so the cloud stays with a coasting bike, and adds no asset file or dependency.
...
The visual gate and the harness that produces the frames an owner signs read **one** scene contract, `scripts/lib/frozen-scene-inventory.mjs`. `e2e/visual-regression.spec.ts` and `scripts/capture-baseline-candidates.mjs` previously each spelled out what "the frozen scene is ready" means — 41 asserted attributes against four waits — so a frame could be captured, reviewed and promoted from a scene state the gate would reject on its next run, and the promotion chain compared everything about those bytes except the state they were taken in. Navigation stays separate on purpose: the two drive different servers with different failure modes. Only the scene is shared.
