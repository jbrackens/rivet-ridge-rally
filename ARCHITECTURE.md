...
Assets shall be original, public-domain, or commercially licensed. `ASSET_LICENSES.md` records source, author, license, modification, attribution, shipped path, and verification status. The build-time asset contract fails if the Blender-authored hero bike-and-rider pipeline, Canyon kit, or shipped Canyon panorama differs from its recorded bytes/structure/provenance markers. The current working-tree contract binds the 24,424-triangle / 283,488-byte hero GLB (18,180 bike triangles, including 6,764 wheel triangles, plus 6,244 rider triangles), SHA-256 `cd38c4b4e892707edc562f61a5c0dfcd5a3ccb8558b1432c25b898eb17ca7765`, to its 15,162-byte manifest, SHA-256 `9468070ec29b43c48847fab4c48108f2439d32760c28d8363b4142f158060ff9`, and source-pair UUID `06059042-a745-437b-8df2-7fcc70e22291`. The asset chain passes verification, focused hero reliability 4/4, focused hero action integration 5/5, the enriched headed action-state capture 15/15, and rival pack 2/2. The retained non-QA `dist/` predates the latest source and remains live for testing rather than being replaced by this QA pass. Immutable-candidate verification, complete source/license review, and human legal/concept acceptance remain separate release gates.
...
Current working-tree command evidence also records passing typecheck and lint; `npm test` with 435 checks (289 Vitest, 44 release-manifest fixtures, 71 release-attestation fixtures, and 31 production-smoke/service-worker fixtures); asset verification for the 24,424-triangle / 283,488-byte hero and its manifest/source-pair binding, rival contract, 32,008-triangle / 427,028-byte Canyon kit, and production art/provenance; `npm audit --audit-level=high` with zero vulnerabilities; and an isolated production-mode Vite compile of 144 modules with only the known 616.11 kB route-chunk advisory. That compile wrote to `/tmp`, left repository `dist/` unchanged, and exposed no active QA build marker. The latest normal build/notices and retained non-QA `dist/` are an earlier checkpoint, not a build of the latest source. These results are not clean-tag or format-2 artifact evidence.

The current non-QA local `dist/` on `127.0.0.1:4173` is a diagnostic checkpoint of 31 files, 7,723,770 raw bytes, and 5,349,548 deterministic gzip bytes. `index.html` is 1,516 bytes with SHA-256 `dfca2229ed68e8faba500495b040aa7cec20a0839d9e1e43ce830b80208afec3`; the inventory aggregate is `5a1b6d4bc0f002c8c1c6304217dd259949cd49e91e6c10518ff8a0a9e765d36e`; and the 43,872-byte notices have SHA-256 `f837ed705667d0f3976bbd419f42d8c63844a3eb4b52f76db88ed3a1d6e270c2`. A scoped live production-preview smoke found no QA API or runtime/request/HTTP errors and the authored hero ready, with the preview process retained for resumed testing. This dirty-tree evidence is not a format-2 manifest, installed/offline smoke, immutable artifact, or release qualification.

The current one-second headed diagnostic at `artifacts/candidate-evidence/diagnostic/performance/repaired-nonqa-lane-latch-4173-headed-20260717T080832Z-60745.json` records stable served bytes and no harness, network, or HTTP errors. Desktop normal/stress measures 59.97/59.99 FPS with 2.2/2.6 ms p95 synchronous frame work, emulated-mobile normal/stress measures 59.95/59.92 FPS with 1.6/1.9 ms p95, and the exact wheelie cue is observed. Its overall result is `FAIL` only on `source-worktree-clean` and `runtime-source-commit-binding`; it does not satisfy the schema-4 release-performance architecture gate or provide physical-device proof.
...
The earlier v2 hero-motion visual contract at `artifacts/visual-review/hero-remodel-2026-07-17-qa-pass-v2/manifest.json` is retained as a 10,268-byte, six-capture historical record with SHA-256 `527327c298edcb2404bf7853611db3e7785336b4888723fabcea67f017a015de`. The current technical contract is the 114,426-byte enriched manifest at `artifacts/visual-review/hero-motion-action-states-v3-headed-20260717t095050z/manifest.json`, SHA-256 `79b9b19f441c7368e1ba495e7903c6a9e3c934eb3348b55a298cb0ab790cc47a`, which records `PASS` for all 15 required states in headed Chromium with browser audio muted, public-UI navigation, authored hero/Canyon/environment readiness, and the deliberate hero fallback. It explicitly records `conceptFidelityClaimed: false`, `visualAcceptance: PENDING_OWNER_REVIEW`, and `baselinePromoted: false`; therefore it is not owner acceptance, a promoted baseline, or legal/concept approval, and the visuals remain **NOT ACCEPTED**.
