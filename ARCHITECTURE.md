...
**Latest-change validation:** `UNVERIFIED` while automated, browser, production-smoke, and release-manifest qualification remains paused
...
The renderer may run while the pre-race gate is paused so a loaded course remains visible. Each engine owns a dedicated Three.js loading manager for its compressed rider. Canyon Kickoff and Rider School additionally own an isolated `ImageBitmapLoader` for the festival-canyon matte. `GameEngine.whenReady()` resolves after both applicable loads have either succeeded or reached their bounded fallbacks. The rider's five-second deadline aborts its manager, terminates its KTX2 worker loader, and selects the already-present built-in bike. The background has its own five-second deadline and retains the already-present generated sky on failure, timeout, disposal, or unsupported `createImageBitmap`. Rejected loads take the same fallback path; any success that still settles during cancellation is disposed instead of replacing the bike or background after readiness has settled. Engine disposal uses the same idempotent cancellation/cleanup paths, so stalled requests and decoded resources cannot survive repeated restarts. React owns only the visible loading/countdown phase and keeps the engine paused through the tutorial intro or non-tutorial countdown. At the sole production engine-construction boundary, `GameView` normalizes tutorial sessions to the canonical Canyon track and masks any retained custom-track payload before deriving either the accessible course identity or the engine options. For a standard race it presents `GO`, yields enough animation frames for the browser to paint that label, and only then hands the engine to the racing phase. The race timer and gameplay input begin together on that fixed-step handoff rather than during asset loading, UI countdown, or an unseen post-`GO` delay.
...
The generated player-bike GLB exposes `FrontTire` and `RearTire` as animated assembly roots so the ring, hub, brake disc, and authored tread blocks spin together. Its rear plate/`22`, layered side panels, and exhaust remain named and source-verified presentation parts. The procedural fallback and every member of the fixed five-entry Rival/Mastery field use the same two complete wheel assemblies, bilateral side panels, and rear-right exhaust/tip. Each procedural bike reuses its wheel geometry/materials between front and rear, while one 128×96 runtime CanvasTexture, material, and plane geometry supply both its jersey and rear-plate number. Rival palette/number data is fixed in `AI_FIELD` as `17`, `31`, `46`, `58`, and `73`; canvas diagnostics expose the number set and shared detail style for deferred inspection.

The runtime retains the procedural rider independently of the loaded player bike. Six named pivots group torso, head, left/right arms, and left/right legs; gloves, boots, and knee pads follow their associated limb. `setRiderPose` reads only existing render-time speed, scalar progress, pitch, lane lean, height, phase, and accessibility values. It applies bounded tuck/cadence, airborne compensation, lean, or one static asymmetric crash pose before the authoritative whole-bike route transform. Reduced Motion resets all nonessential joint transforms and bob while preserving the static crash silhouette. These objects and transforms never feed simulation, collision, AI, input, timing, classification, or replay state. The fixed field size and per-bike shared geometry/material/number resources bound allocation growth; runtime draw cost and racing-distance readability remain performance/browser qualification gates rather than architectural claims.
...
Custom-track schema v2 adds only the optional checkpoint route anchor. Dexie database version 5 (native IndexedDB version 50) upgrades structurally valid v1 custom-track roots that meet current semantics by changing only `schemaVersion`; all modules, identifiers, timestamps, and thumbnail bytes remain untouched. A v1 track accepted under the former center-only route boundary but needing current full-parent-footprint repair remains stored as v1 and is normalized to editable v2 data in memory without quarantine; current save/Test Ride/export remains blocked until repair. V1 JSON imports use the same compatibility path, while successful save/export always emit v2. Future versions fail closed and remain in place. The native-version barrier prevents an older v4 build from opening and quarantining records written by the v2-aware application.
...
- Offline behavior reflects what is actually cached; no false “offline ready” claim is permitted. The page imports both lazy race/editor entry points, then accepts readiness only from an acknowledgement whose cache identity matches the expected generation and whose sender is still the page's current service-worker controller. Every later `controllerchange` synchronously clears the page's readiness datasets and notifies subscribed UI; readiness stays conservative until the replacement controller completes a fresh exact-generation acknowledgement.
- The current service worker is `rivet-ridge-rally-shell-v30`. Install fetches `index.html` once with `cache: "reload"`, clones that response under `/` and `/index.html`, and fetches every core asset—including both bundled display-font subsets—plus each hashed asset discovered from the HTML before writing the batch. If any fetch or cache write fails, the worker deletes the partial v30 cache and rejects installation. Runtime-resource warming also uses reload fetches, but may accept an already cached matching URL when the refresh fails.
- Cache lookup prefers v30 and may fall back only to the immediately previous app-owned transition cache. Activation removes older app generations while leaving unrelated origin caches untouched. This transition window prevents an old open tab from losing its hashed lazy assets without allowing stable URLs from the previous generation to override current bytes.
...
Current evidence and any remaining exceptions are recorded requirement-by-requirement in `QA_REPORT.md`; the latest gameplay, responsive, accessibility, typography, shell-v30, and manifest-bound smoke changes remain `UNVERIFIED`, and acceptance must not be inferred from this document.
