...
| 3. Representative visual vertical slice | Open | `GAME_BIBLE.md`, `GAME_SPEC.md`, `docs/design/FIDELITY_LEDGER.md`, `docs/HANDOFF_TO_CLAUDE_FABLE.md` | Define and implement the smallest slice that proves the revised direction: hero bike/rider, representative track section, terrain/materials, jump or barrier, festival dressing, lighting/atmosphere, motion/VFX feedback, HUD readability, and performance evidence. | Scoped implementation, comparison captures, asset/bundle measurements, and a review manifest tied to the exact commit. | Yes. | Owner input is required to accept the direction after the slice is prepared. | Slice demonstrates an original cohesive direction, preserves gameplay readability, meets the documented performance profile, and is ready for owner review. |
...
Start with gates 1 and 2. Confirm this checklist matches the current source and that the creative pivot has reached every style guide or design reference that governs visual acceptance. Then complete gate 3 as a contained visual proof, prepare the gate 4 owner-review package, and close the missing structured QA records. Do not attempt baseline promotion or final attestation before the prerequisite approvals and evidence exist.
