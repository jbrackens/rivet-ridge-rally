...
The current non-QA local `dist/` served on `127.0.0.1:4173` contains 33 files totaling 7,993,780 raw bytes. `index.html` is 1,632 bytes with SHA-256 `f13a52182ecc870bcab24e3a60fe81b34ab02d84ab63653ac998562a12f5d455`; `assets/3d/hero-bike-rider.glb` is 511,736 bytes with SHA-256 `5537ba34639053a1b28b40978853ceade33f9d3ac40d2d2c477e59ce206c024a`; the inventory aggregate is `6ca0548d17afa9993c1a8064bae2facc765c4817f7022feb7bc200af5e825524`; and the 43,872-byte notices have SHA-256 `f837ed705667d0f3976bbd419f42d8c63844a3eb4b52f76db88ed3a1d6e270c2`. A live HTTP sanity check confirmed preview PID 53866 serves the rebuilt `assets/index-Epo_xwqQ.js`, `assets/index-C2e4audF.css`, `assets/GameView-RsjCVCzd.js`, `assets/TrackEditorScreen-DNvFsdrN.js`, and `assets/CoursePresentationRoute-BmJAzo7O.js` bytes. This is a local dirty-tree checkpoint, not a format-2 manifest, installed/offline production smoke, immutable artifact, or release qualification.
...
Release-provenance hardening is implemented in source and remains **UNVERIFIED** as a real candidate gate. The guard captures clean `HEAD`, creates its detached checkout, validates the exact annotated tag, npm package tree, Node identity, clean inputs, and isolated output, and fails closed on unsafe/special entries or mutation. Current `npm run test:release-manifest` passes 45/45; historical format-1 evidence remains unchanged. A real `npm run visual:candidate` attempt on 2026-07-18 correctly exited 1 with `Release guard failed: Git working tree is not clean`, and a later clean-branch attempt exposed and corrected an overbroad binary token-prefix false positive. The latest clean `71e902ccaded9a1ca9cfd2157333881ea733ac2d` source produced visual QA candidate aggregate `95dfb6c69360085cfbc905d689b00d05937b3d59f5c4abaea8a78c89a357431b`, candidate manifest SHA-256 `b0f91f89f8bcd340abd5d1ef5e254ca90a337a33dba08820c309328e5f78c4b5`, and `artifacts/visual-review/rc2-five-track-controlled-71e902c/manifest.json` records 11/11 `PASS` with SHA-256 `c99a656a311945ff9467a2a8dbc41ea6a929b54282451c6e135e117907d48801`.
...
The current normal working-tree build generates and verifies its notices without the former Vite chunk-size advisory after the Three manual-chunk split. Production-smoke source rejects unsafe screenshot names, binds screenshot bytes/hashes, checks evidence-root symlink ancestors before writes, and the release bundle guard rejects local paths, private-key headers, and live-token prefixes. Fixture coverage passes, but no real format-2 manifest, production smoke, or attestation exists for the current working tree.
...
Latest visual-candidate rehearsal: `npm run visual:candidate` was attempted on 2026-07-18 and failed before writing candidate bytes because the working tree was not clean. A later clean-branch attempt reached the isolated `dist/` scan and exposed an overbroad `ghp_` byte-prefix check against the binary Canyon GLB. The follow-up `npm run test:release-manifest` passed 45/45 after adding the short-token-prefix regression. The latest clean `71e902c` `visual:candidate` produced a 33-file / 7,997,693-byte visual QA candidate with aggregate `95dfb6c69360085cfbc905d689b00d05937b3d59f5c4abaea8a78c89a357431b`; `visual:capture` wrote `artifacts/visual-review/rc2-five-track-controlled-71e902c/manifest.json`, passed 11/11, and recorded manifest SHA-256 `c99a656a311945ff9467a2a8dbc41ea6a929b54282451c6e135e117907d48801`. The correct next action is owner/legal visual review plus guarded baseline promotion, not another blind capture retry.
