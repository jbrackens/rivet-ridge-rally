...
Latest dirty-tree rehearsal: `npm run visual:candidate` was attempted on 2026-07-18 and failed before writing candidate bytes because the working tree was not clean. The follow-up `npm run test:release-manifest` passed 44/44, so the correct next action is source/evidence inventory plus a clean commit, not a blind capture retry.
