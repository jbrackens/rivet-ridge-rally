...
`1.0.0-rc.2` is **not qualified for deployment**. The canonical workspace uses the neutral `Rivet Ridge Rally` directory name. The current source is on an ordinary development branch and is not pinned by an annotated `v1.0.0-rc.2` tag; no clean qualified release-bound rebuild exists. Current working-tree typecheck, lint, all 444 tests/fixtures (297 Vitest, 45 release-manifest, 71 release-attestation, and 31 production-smoke/service-worker), asset verification, zero-vulnerability high-severity audit, `npm run build`, and the scoped browser evidence recorded in `QA_REPORT.md` pass. Those results do not replace a frozen-candidate run. The annotated `v1.0.0-rc.1` source tag and recorded evidence remain immutable historical records and must not be rebuilt or relabeled; deployable predecessor artifact retrieval is separately **UNVERIFIED**.

Current cross-project scoped evidence is campaign modes 18/18 in 20.0 minutes, rival pack 2/2, 10/10 Firefox-plus-WebKit desktop functional, 6/6 WebKit lifecycle, 6/6 emulated touch matrix, and 2/2 emulated touch tutorial intro, in addition to the focused Chromium functional/reliability/tutorial/editor/input/migration/persistence/quality paths. These are working-tree runner results, not installed-browser, physical-device, frozen-candidate, or commercial-release evidence.

The current focal player asset is the original Blender-authored Hero Bike and Rider A; the shared rival pack and Canyon kit use the recorded generated/provenance chains in `ASSET_LICENSES.md`. `npm run assets:build && npm run assets:verify` passes the 42,360-triangle / 433,420-byte hero GLB (35,300 bike triangles, including 14,124 wheel triangles, plus 7,060 rider triangles), SHA-256 `2a286cf6af819526155416dbae026e0658cdfcfdf3a08ddb132c8370f4c4198c`; its 15,164-byte manifest, SHA-256 `dd08c3befd74c11cd664a33fa8128b9ed2cdbe79abaca6c77eca1912df2b1698`; source-pair UUID `71481acc-6e2a-4884-91f3-622b7c0b962e`; the rival contract; Canyon 32,008 triangles / 427,028 bytes; and production art/provenance including the v3 action reference. Focused hero reliability passes its latest authored-Canyon/hero readiness case 1/1 on isolated QA port 4179, focused hero action integration passes 5/5, the enriched v12 headed action-state capture passes 15/15 for the current runtime with material/shadow/dust readiness checks, and rival pack passes 2/2. Required additions remain untracked, and offline, performance, immutable-source qualification, complete legal review, and human similarity acceptance remain pending or **UNVERIFIED**.
...
The earlier v2 six-capture, v3 action-state, v8 detail, and v9 hero-motion reviews remain preserved as historical evidence. The current technical review is the 121,461-byte manifest at `artifacts/visual-review/hero-motion-action-states-v12-current-20260718t192834z/manifest.json`, SHA-256 `2e155c59dc9d29aaa50c48e9532de7101fd5690bc7fdf5c0edd7c0c499b400c7`; it records `PASS` for all 15 required states in headed Chromium with browser audio muted, no persisted audio-setting change, and material/shadow/dust readiness checks for the current runtime. This is still dirty-working-tree, nonbaseline technical evidence: `conceptFidelityClaimed` is false, visual acceptance is pending owner review, and baseline promotion is false. The visuals are **NOT ACCEPTED**, and owner concept-art/similarity and legal approval remain open.
...
An earlier normal `rc.2` snapshot passed its recorded build, performance, soak, dependency-audit, format-1 manifest, and installed-Chrome smoke scopes. It does **not** match or qualify the current `dist/` or candidate. Current focused browser evidence, the zero-vulnerability working-tree audit, and `npm run build` of the latest source pass separately; the build reports only the known `CoursePresentationRoute` 616.11 kB minified-chunk warning above the 600 kB advisory and regenerates notices. Exact-product performance, soak, format-2 manifest, production/offline smoke, and attestation remain pending. Before any `rc.2` deployment, regenerate the complete release evidence against one clean annotated candidate.

The current non-QA local `dist/` served on `127.0.0.1:4173` contains 31 files totaling 7,893,385 raw bytes and 5,415,977 deterministic gzip bytes. `index.html` is 1,516 bytes with SHA-256 `3ae86d1940a3225b74f4d68509195f78c2841c91533fa86a1b6a7eb499f64821`; `assets/3d/hero-bike-rider.glb` is 433,420 bytes with SHA-256 `2a286cf6af819526155416dbae026e0658cdfcfdf3a08ddb132c8370f4c4198c`; the inventory aggregate is `0a0b06c02847715f675f912a2f10484fa940b12d9a6a941ceeed6e41c6fd9e2f`; and the 43,872-byte notices have SHA-256 `f837ed705667d0f3976bbd419f42d8c63844a3eb4b52f76db88ed3a1d6e270c2`. A scoped live production-preview smoke was clean, exposed no runtime/request/HTTP/console/page errors, and reported the authored hero, Canyon kit, environment, PMREM three-point material response, contact-shadow diagnostics, and `trail-burst-contact` dust style ready; preview PID 92618 remains retained for resumed testing. This is a local dirty-tree checkpoint, not a format-2 manifest, installed/offline production smoke, immutable artifact, or release qualification.

The current one-second headed diagnostic at `artifacts/candidate-evidence/diagnostic/performance/repaired-nonqa-lane-latch-4173-headed-20260717T080832Z-60745.json` reports no harness, network, or HTTP errors and stable served bytes. Desktop normal/stress measured 59.97/59.99 FPS with 2.2/2.6 ms p95 synchronous frame work; emulated-mobile normal/stress measured 59.95/59.92 FPS with 1.6/1.9 ms p95; and the exact wheelie cue was observed. Its overall status is `FAIL` only because `source-worktree-clean` and `runtime-source-commit-binding` fail. It is diagnostic evidence, not schema-4 release performance qualification or physical-device proof.
...
Release-provenance hardening is implemented in source and remains **UNVERIFIED** as a real candidate gate. The guard captures clean `HEAD`, creates its detached checkout, validates the exact annotated tag, npm package tree, Node identity, clean inputs, and isolated output, and fails closed on unsafe/special entries or mutation. Current `npm run test:release-manifest` passes 45/45; historical format-1 evidence remains unchanged. A real `npm run visual:candidate` attempt on 2026-07-18 correctly exited 1 with `Release guard failed: Git working tree is not clean`, and a later clean-branch attempt exposed and corrected an overbroad binary token-prefix false positive. The clean `a68ac6d94de81b2e2b2e35a9ae9d9dbeb63f3a37` source then produced visual QA candidate aggregate `3dd15de460fa49084e78104639e0a6ee970758b01624e3cb50497af22bd37314`, and `artifacts/visual-review/rc2-five-track-controlled-a68ac6d/manifest.json` records 11/11 `PASS` with SHA-256 `12546b275e437054b6fd029fb80742f96b9aaded7012a9027ce675a140f3864c`.

The current focused browser suite exercises the safe Ride/heat path, PB/results flow, campaign/Rider School start, portrait/sub-680 presentation, touch/Recovery semantics, and responsive UI in its stated scopes. Campaign modes pass 18/18 in 20.0 minutes, rival pack passes 2/2, and the complete working-tree unit/fixture command passes. Accepted visuals, exact-product performance, frozen-candidate repetition, and physical devices remain **UNVERIFIED**.
...
The current normal working-tree build generates and verifies its notices, with only the known `CoursePresentationRoute` 616.11 kB minified-chunk warning above the 600 kB advisory. Production-smoke source rejects unsafe screenshot names, binds screenshot bytes/hashes, checks evidence-root symlink ancestors before writes, and the release bundle guard rejects local paths, private-key headers, and live-token prefixes. Fixture coverage passes, but no real format-2 manifest, production smoke, or attestation exists for the current working tree.
