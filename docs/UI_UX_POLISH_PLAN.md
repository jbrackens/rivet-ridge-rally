...
| 4.4 | Lighthouse measurement | **Resolved as local-only 2026-07-20:** a CI `lighthouse-report` job was built and removed after three diagnosed failures — Lighthouse's devtools-log gatherer streams full response bodies (incl. the 2 MB title art) over CDP, which deterministically exceeds the protocol timeout on 2-core runners (`Network.getResponseBody`), and that gatherer is not prunable from the CLI. Local runs work and are the repeatable path: `npm run preview -- --port 4186 & npx -y lighthouse http://127.0.0.1:4186 --preset=desktop --skip-audits=<byte-efficiency list> --chrome-flags=--headless=new`. **Baseline 2026-07-20 (M-series, desktop preset): performance 54 (TBT 2,240 ms is the driver; LCP 1.9 s; CLS 0.058), accessibility 100, best-practices 100, seo 91.** The lighthouse MCP server in `.mcp.json` covers in-session measurement. Next perf lever per the baseline: boot-time TBT, not payload. | Local report JSON; scores recorded here |
